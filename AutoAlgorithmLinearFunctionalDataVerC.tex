...
\end{equation}
Moreover, $\textup{cost}(\widetilde{A},{\color{red}\mathcal{C},}\varepsilon,\rho) \le n_{j^\dagger}$, where $j^\dagger$ satisfies the following upper bound:
...
For the remainder of the proof consider $\rho$ and $\varepsilon$ fixed.  To derive an upper bound on $n_{j^\dagger} = \textup{cost}(\widetilde{A},{\color{red}\mathcal{C},}\varepsilon,\rho)$ we first note some properties of $\sigma_j(f)$ for all $f \in \calC$:
