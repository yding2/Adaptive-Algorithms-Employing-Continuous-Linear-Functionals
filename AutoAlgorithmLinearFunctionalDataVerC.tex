...
\abstract{Automatic algorithms attempt to provide approximate solutions that differ from exact solutions by no more than a user-specified error tolerance. This paper describes an automatic, adaptive algorithm for approximating the solution to a general linear problem on Hilbert spaces. The algorithm employs continuous linear functionals of the input function, specifically Fourier coefficients. We assume that the Fourier coefficients of the solution to be approximated decay sufficiently fast, but do not require the decay rate to be known a priori. We also assume that the Fourier coefficients decay steadily, although not necessarily monotonically. Under these assumptions, our adaptive algorithm is shown to produce an approximate solution satisfying the desired error tolerance, without prior knowledge of the norm of the function to be approximated. Moreover, the computational cost of our algorithm is shown to be essentially no worse than that of the optimal algorithm. We provide a numerical experiment to illustrate our algorithm.}
...
\tonynote{It looks like the font size in Figure 2 is smaller than that of Figure 1. Perhaps it would be easier to read if a bit larger?}

\tonynote{In the x-axis of Figure 2, we mention $\hat{f}$. Is it referring to the Fourier coefficients? As a suggestion, are we not interested in something like $\DHJRnorm[\mathcal{G}]{S(f)-\widetilde{A}({f},\varepsilon)}$? This could allow us to plot marks at the different $\varepsilon$ and see how conservative the algorithm is.}
