...
\end{gather*}
Note that $\textup{comp}(\calA(\calH),\varepsilon,\rho) \ge \textup{comp}(\calA(\calH \cap \calB_{\rho}),\varepsilon)$ ({\color{red} $\leq$ ?}) \frednote{The inequality is correct.  The former problem is harder}.  In the former case, the algorithm is unaware  that the input function has norm no greater than $\rho$.  
...
\begin{example} Consider the case of function approximation for periodic functions defined over [0,1], and algorithm $\widehat{A}$ defined in \eqref{optBallalg}:
	...
	\end{align*}
	{\color{red} Perhaps worth mentioning what norm makes $\widehat{\lambda}_k\widehat{v}_{k}$ orthonormal in $\mathcal{F}$.} \frednote{They are orthonormal because we define the norm that way.}
...
Our goal is not only to construct algorithms in $\calA(\calH)$ for some $\calH$, but also to determine whether the computational cost of our algorithms is reasonable.  We define an algorithm $A \in \calA(\calH)$ to have \emph{essentially no worse cost}  than an algorithm $A^* \in \calA(\calH^*)$ if for some number $\omega$,
\begin{gather}
...
\end{gather}
If these inequalities are not satisfied, we say that $A$ has \emph{essentially worse cost}  than $A^*$ .
...
\end{equation}
Moreover, $\textup{cost}(\widetilde{A},\calC,\varepsilon,\rho) \le n_{j^\dagger}$, \frednote{Tony, I think you misunderstood.  It was correct to begin with.} where $j^\dagger$ satisfies the following upper bound:
