...
\end{gather}
\end{subequations}
This setting includes, for example, the recovery of functions, derivatives, indefinite integrals, and solutions of linear (partial) differential equations.  We focus on cases where the exact solution generally requires an infinite number of series coefficients, $\widehat{f}_i$. Thus, this setting is not relevant for evaluating the definite integral or any other linear functional, since in those cases $\lambda_2 = \lambda_3 = \cdots = 0$. {\color{red} The embedding operator is not considered either, right ($\lambda_i=1$)? When do these two basis exist? For instance, can we freely choose from exponential to Walsh bases?}

\subsection{Solvability}  Let $\calH$ be any subset {\color{red}(any subset? But $S$ will linearly defined over $\calH$?)} of $\calF$, and let $\calA(\calH)$ denote the set of deterministic algorithms that successfully approximate the solution operator $S : \calH \to \calG$ to within some error tolerance for all inputs in $\calH$:
...
\widehat{u}_{\bsk} (\bsx) &:= \prod_{j=1}^d \frac{2^{(1-\delta_{k_j,0})/2}\cos(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 )}{\max(1,\gamma_j \DHJRabs{k_j})},  \\ 
...
\widehat{v}_{\bsk}(\bsx) & : =  -\textup{sign}(k_1) 2^{(1-\delta_{k_1,0})/2} \sin(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 ) \\
& \qquad \qquad \times \prod_{j=2}^d 2^{(1-\delta_{k_j,0})/2} \cos(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 ), \\ 
\lambda(\bsk) &:= \frac{2 \pi \DHJRabs{k_1}}{\prod_{j=1}^d\max(1,\gamma_j k_j)}, \\
\boldsymbol{\gamma} & := (1,1/2, 1/4, \ldots, 2^{-d+1}).
\end{align*}

\yuhannote{According to code, some expression should be as follows:}
\frednote{I hope that it is fixed now.}
\begin{align*}
u_{\bsk} (\bsx) &:= \prod_{j=1}^d \frac{2^{(1-\delta_{k_j,0})/2}\cos(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 )}{\max^8(1,\gamma_j k_j)},  \\ 
\lambda(\bsk) &:= 2 \pi \DHJRabs{k_1}\frac{2^{(1-\delta_{k_j,0})/2}}{\prod_{j=1}^d\max^8(1,\gamma_j k_j)}.\\
\end{align*}

\yuhannote{$v_{\bsk}(\bsx) $ still doesn't match the code}
\begin{align*}
    v_{\bsk}(\bsx) & : =  -\textup{sign}(k_1)  \sin(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 ) \\
& \qquad \qquad \times \prod_{j=2}^d
\cos(2\pi k_j x_j  + \bbone_{(-\infty,0)}(k_j) \pi/2 ) 
\end{align*}


We construct a function by choosing its Fourier coefficients $\widehat{f}(\bsk) \overset{\text{IID}}{\sim}  \mathcal{N}(0,1)$ for  $\bsk \in \{-30, -29, \ldots, 30\}^d$, and $\widehat{f}(\bsk) = 0$ otherwise.  This corresponds to $61^3 \approx 2 \times 10^5$ nonzero Fourier coefficients.  Let $ a= 2$ and  $b=\frac{1}{2}$ and 
...
Fig. \ref{errfig} shows the sample size needed for ten different error tolerances from $10^{-1}$ to $10^{1}$. If we consider $\varepsilon = 0.1$, $ n_{j^\dagger} = 8192$ is sufficient to satisfy the error tolerance. For $\varepsilon = 0.1$, the approximate solution and the error in Figure \ref{solfig}. Based on Fig. \ref{solfig}(b), we can obtain the exact errors are less than $0.01 < \varepsilon $.  This shows our algorithm works.
