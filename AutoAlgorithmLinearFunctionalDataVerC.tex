...
\end{align*}
where $\gamma = [2^0, 2^{-1},\ldots, 2^{-d+1}].$
...
\lambda_{\bsk} &:= 2 \pi \DHJRabs{k_1}\frac{2^{(1-\delta_{k_j,0})/2}}{\prod_{j=1}^d\max^8(1,\gamma_j k_j)}.\\
\end{align*}
...
We construct a function by choosing its Fourier coefficients $\widehat{f}_i \overset{\text{IID}}{\sim}  \mathcal{N}(0,1)$ for  $i = 1, \ldots, 68931$, and $\widehat{f}_i = 0$ otherwise.  Let $ a= 2$ and  $b=\frac{1}{2}$ and 
 choose $\bsn = \{0, 2^{3+1}, \ldots, 2^{3+k}, \ldots \}.$
 To  obtain $\sigma_j(f), \ j \in \bbN$ by \eqref{sumdef},
 we need to sort $\lambda_i$ in the descending order and get corresponding $\widehat{f}_i$ and $\widehat{v}_i$, which is similar to the Example 1. To implement it efficiently, we use another vector to keep the index of the descending order of $\lambda_i$. Thus, we can obtain the correct
 $\lambda_i$,  $\widehat{f}_i$, and $v_i$ according to $\bsn_k$. Thus, we can add them together to get $\sigma_j(f)$.  Given $\varepsilon$, we can derive the error bound and find the $n_{j^\dagger}$ where
...
\begin{figure}[ht]
	...
	\begin{tabular}{cc}
		...
		\\ (a) & (b)
	\end{tabular}
	\caption{For $\varepsilon = 0.1$:  (a) The approximation of the first partial derivative of $f$; 
